...
\begin{frame}{What is a query service?}
\large
\vspace{0.5 cm}
...
\item Should represent multiple, related histograms, fillable en masse: ``books'' of histograms. Ideally, those relationships should be passed on to a multi-channel fitter.
\item Also needs to include small ntuples (TODO).
\end{itemize}
\end{frame}
...
\item Still motivated by the idea of a query service, although that's a big problem with many parts; probably a lot of smaller projects will have to come together.

\item I've been concentrating on the columnar array processing part of that because it's so underdeveloped (inside and outside of HEP).
