...
\begin{frame}[fragile]{Like Femtocode without zero-runtime-error guarantee}
\vspace{0.5 cm}
Only four types of arrays are needed to make a reasonably complete typesystem ({\bf PLUR}, as expressive as Protobuf/Avro/Parquet/Arrow):
\begin{itemize}
\item {\bf P}rimitive: plain Numpy arrays
\item {\bf L}ist: jagged arrays
\item {\bf U}nion: tagged, indexed list of arrays
\item {\bf R}ecord: mapping from names to arrays
\end{itemize}
...
\begin{itemize}
\item {\bf P}ointer: indexed array
\end{itemize}
\end{uncoverenv}

\vspace{0.25 cm}
\uncover<3->{And the ability to mix-in physics methods to table rows, tables, and jagged tables:}
...
\begin{uncoverenv}<7->
\begin{minted}{python}
boosted_jets = jets.boost(leading.boostvector())
\end{minted}
\end{uncoverenv}
\end{frame}
